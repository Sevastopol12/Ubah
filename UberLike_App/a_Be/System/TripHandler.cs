...
namespace Ubah
{
    public class TripHandler
    {
        ...
        public async Task<Trip> CreateTrip(Rider customer, Dictionary<string, object> pickup, Dictionary<string, object> dropoff)
        {
            ...
            Task<Dictionary<string, object>> validPickup = this.geoservice.ValidateLocation(location: pickup);
            pickup = await validPickup;

            Task<Dictionary<string, object>> validDropoff = this.geoservice.ValidateLocation(location: dropoff);
            dropoff = await validDropoff;

            this.trip.AssignLocation(pickupLocation: pickup, dropoffLocation: dropoff);
